...
class Loader(object):
    ...
    def loadstring(self, name, templatestring):
        if name not in self.templates:
            self.templates[name] = Template( templatestring, name=name, loader=self)
...
class _Codeblock(_Node):
    ...
    def generate(self, writer):
        ...
        writer.write_line(  '''code = """%s"""\n''' % self.code  )
        writer.write_line(  "sys.stdout = _out = cStringIO.StringIO()\n")
        writer.write_line(  "try: exec code\n" )
        writer.write_line(  "except:\n")
        writer.write_line(  "    sys.stdout = sys.__stdout__\n" )
        writer.write_line(  "    raise\n" )
        writer.write_line(  "sys.stdout = sys.__stdout__\n" )
        writer.write_line(  "s = _out.getvalue()\n" )
        writer.write_line(  "_out.close()\n" )
        writer.write_line(  "if s: _buffer.append( s)\n" )
...
def _parse(reader, in_block=None):
    ...
    while True:
        ...
        if start_brace == "{{":
            ...
            if contents.find("\n") != -1:
                body.chunks.append(_Codeblock(contents))
            else:
